...
def recomd():
    ...
    file_path = 'home_user_activity.csv'  
    data = pd.read_csv(file_path)

   
    data['timeSpent'] = data['timeSpent'].fillna(0)

  
    filtered_data = data[~data['element'].str.contains('BUTTON', case=False, na=False)]
    filtered_data = filtered_data[~filtered_data['event'].str.contains('scroll', case=False, na=False)]

   
    interaction_summary = filtered_data.groupby(['element', 'event'])['timeSpent'].sum().reset_index()

   
    interaction_summary = interaction_summary.sort_values(by='timeSpent', ascending=False)

  
    top_recommendations = interaction_summary.head(5)


    print(top_recommendations)

  
    return render_template('recomd.html', recommendations=top_recommendations)
